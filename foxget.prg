...
* Download each file. Start by connecting to the URL.
		...
		for each loFile in This.oFiles foxobject
			...
			if lhURL = 0
				lcMessage = lcURL + ' cannot be opened'
				raiseevent(This, 'Update', lcMessage)
				This.Log(lcMessage)
				llReturn = .F.
				exit
...
* Write the download file and log the results.
			...
			if llReturn
				try
					...
					llReturn = .F.
					raiseevent(This, 'Update', 'Error saving file: see log file for details')
					This.Log('Error saving file: ' + loException.Message)
				endtry
			else
				exit
			endif llReturn
...
* cranky if the files already exist.
		...
		if directory(This.cExtractionPath)
			try
				This.FileOperation(This.cExtractionPath, '', 'DELETE')
			catch to loException
...
*==============================================================================
	...
	function FileOperation(tcSource, tcDestination, tcAction, tlUserCanceled, ;
		tlFilesOnly, tlQuiet)
		local loHeap, ;
			lcAction, ;
			laActionList[1], ;
...
*** TODO: put into it's own file since we need to build an app

* Ed Rauh's API_AppRun class.
...
*	Not a character expression - ignore
				Case Adir(aDirTest, tcLaunchDir, 'D') = 0
